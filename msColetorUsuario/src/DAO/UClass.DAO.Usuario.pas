...
begin
  ...
  begin
     ...
     CreateField(
        ...
        'USU_TIPO',
        1,
        'ALTER TABLE USUARIO ADD USU_TIPO VARCHAR(1) DEFAULT ''R'' NOT NULL'
     );
  end;
end;
...
begin
   ...
   try
      try
         LQuery.Close;
         LQuery.SQL.Clear;
         LQuery.SQL.Text := ' INSERT INTO USUARIO (USU_CODIGO, USU_EMAIL, USU_CPF, USU_SENHA, USU_TIPO)      '+
                            '              VALUES (:USU_CODIGO, :USU_EMAIL, :USU_CPF, :USU_SENHA, :USU_TIPO) ';
         ...
         LQuery.ParamByName('USU_TIPO').Value   := ConstTipoUsuario[aModel.Tipo];
         LQuery.ExecSQL;
...
begin
   ...
   Result.Tipo   := StrToTipoUsuario(aQuery.FieldByName('USU_TIPO').AsString);
end;
   ...
   try
      try
         ...
         if not aSenha.IsEmpty then
            LWhere := LWhere + ' AND USU_SENHA = ' + QuotedStr(aSenha);
